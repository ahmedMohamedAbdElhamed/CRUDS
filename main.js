...
function readData(){
    ...
    table = '';
    for(let i = newProduct.length; i > 0; i--){
        table += `<tr>`;
        table += `<td>${i-1 + 1}</td>`;
        table += `<td>${newProduct[i-1].prodTitle}</td>`;
        table += `<td>${newProduct[i-1].prodPrice}</td>`;
        table += `<td>${newProduct[i-1].prodTaxes}</td>`;
        table += `<td>${newProduct[i-1].prodAds}</td>`;
        table += `<td>${newProduct[i-1].prodDis}%</td>`;
        table += `<td>${newProduct[i-1].prodTotal}</td>`;
        table += `<td>${newProduct[i-1].prodCount}</td>`;
        table += `<td>${newProduct[i-1].prodCatg}</td>`;
...
function updateItem(i){
    ...
    title.value = newProduct[i-1].prodTitle;
    price.value = newProduct[i-1].prodPrice;
    taxes.value = newProduct[i-1].prodTaxes;
    ads.value = newProduct[i-1].prodAds;
    discount.value = newProduct[i-1].prodDis;
    count.value = newProduct[i-1].prodCount;
    total.innerHTML = newProduct[i-1].prodTotal;
    catg.value = newProduct[i-1].prodCatg;
    namesID = title.value;
